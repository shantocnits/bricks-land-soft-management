<div class="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 px-4 sm:px-6 py-3 rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 transition-colors duration-300">
        <div>
            <h2 class="font-bold text-gray-800 dark:text-white text-sm leading-tight">à¦¸à¦¬ à¦šà¦¾à¦²à¦¾à¦¨</h2>
            <p class="text-[10px] text-gray-400 dark:text-gray-500 font-sans mt-0.5 font-semibold">à¦¸à¦•à¦² à¦šà¦¾à¦²à¦¾à¦¨à§‡à¦° à¦¸à¦®à§à¦ªà§‚à¦°à§à¦£ à¦¤à¦¾à¦²à¦¿à¦•à¦¾ à¦“ à¦°à¦¿à¦ªà§‹à¦°à§à¦Ÿ</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 flex-wrap">
            <!-- Search -->
            <div class="relative">
                <input type="text" wire:model.live="search" placeholder="à¦¸à¦¾à¦°à§à¦š à¦•à¦°à§à¦¨..."
            ...
            <input type="date" wire:model.live="dateFrom"
                   class="py-2 px-3 text-xs rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-sans font-semibold">
            <input type="date" wire:model.live="dateTo"
                   class="py-2 px-3 text-xs rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-sans font-semibold">
            <!-- Type Filter Tabs -->
            <div class="flex items-center bg-gray-100 dark:bg-slate-800 rounded-xl p-1 gap-1">
                <button type="button" wire:click="$set('filterType', 'all')"
                        class="px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer font-sans {{ $filterType === 'all' ? 'bg-white dark:bg-slate-700 text-gray-850 dark:text-white shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white' }}">
                    à¦¸à¦¬
                </button>
                <button type="button" wire:click="$set('filterType', 'today')"
                        class="px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer font-sans {{ $filterType === 'today' ? 'bg-white dark:bg-slate-700 text-gray-850 dark:text-white shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white' }}">
                    à¦†à¦œà¦•à§‡à¦°
                </button>
                <button type="button" wire:click="$set('filterType', 'pending')"
                        class="px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer font-sans {{ $filterType === 'pending' ? 'bg-white dark:bg-slate-700 text-gray-855 dark:text-white shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white' }}">
                    à¦…à¦—à§à¦°à¦¿à¦®
                </button>
            </div>
            <!-- Report -->
                ...
                <table class="w-full text-left border-collapse border border-gray-200 dark:border-slate-800" style="min-width: 1100px">
                    <thead>
                        <tr class="bg-emerald-600 text-white text-[11px] font-bold uppercase font-sans">
                            ...
                            <th class="px-3 py-3 border-r border-white/20 last:border-r-0">à¦¤à¦¾à¦°à¦¿à¦–</th>
                            ...
                            <th class="px-3 py-3 text-right border-r border-white/20 last:border-r-0">à¦¬à¦¾à¦•à¦¿</th>
                            <th class="px-3 py-3 text-center border-r border-white/20 last:border-r-0">à¦§à¦°à¦¨</th>
                        ...
                        @forelse($challans as $challan)
                            <tr class="hover:bg-emerald-50/40 dark:hover:bg-emerald-950/10 transition-colors text-xs">
                                ...
                                <td class="px-3 py-3.5 text-center border-r border-gray-150 dark:border-slate-800 last:border-r-0">
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $challan->challan_type === 'à¦†à¦œà¦•à§‡à¦°' ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border border-emerald-100' : 'bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 border border-amber-100' }}">
                            ...
                            <tr>
                                <td colspan="16" class="px-4 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <p class="text-xs text-gray-400 dark:text-gray-500 italic font-sans">à¦•à§‹à¦¨à§‹ à¦šà¦¾à¦²à¦¾à¦¨ à¦ªà¦¾à¦“à¦¯à¦¼à¦¾ à¦¯à¦¾à¦¯à¦¼à¦¨à¦¿à¥¤</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                    ...
                    <tfoot>
                        <tr class="bg-emerald-600/10 dark:bg-emerald-950/30 border-t-2 border-emerald-200 dark:border-emerald-900/50 text-xs font-bold font-sans">
                            <td colspan="5" class="px-3 py-3 text-emerald-700 dark:text-emerald-400 border-r border-gray-200 dark:border-slate-800 last:border-r-0">à¦®à§‹à¦Ÿ à¦šà¦¾à¦²à¦¾à¦¨ {{ $challans->count() }} à¦Ÿà¦¿</td>
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3 mb-6">
                        ...
                        <div class="ml-auto flex items-center gap-2">
                            <div class="flex items-center gap-1.5 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-850 rounded-xl px-3 py-1.5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                        <div>
                            ...
                            <select wire:model="challan_type" class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-855 bg-gray-50 dark:bg-slate-950 text-xs text-gray-850 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="notes" placeholder="à¦¨à§‹à¦Ÿ..." class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-850 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        </div>
                    </div>
                                ...
                                @foreach($items as $index => $item)
                                    <tr class="text-xs">
                                        ...
                                        <td class="px-3 py-2.5 border-r border-gray-150 dark:border-slate-850">
                                            <input type="number" step="0.01" wire:model.live="items.{{ $index }}.rate" placeholder="à§³ à§¦" class="w-full py-1 px-2 rounded-lg border border-gray-250 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-right text-xs font-semibold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all text-gray-800 dark:text-white">
                                        </td>
                                        <td class="px-3 py-2.5 border-r border-gray-150 dark:border-slate-850">
                                            <input type="number" wire:model.live="items.{{ $index }}.quantity" placeholder="à§¦" class="w-full py-1 px-2 rounded-lg border border-gray-250 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-right text-xs font-semibold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all text-gray-800 dark:text-white">
                                        </td>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 items-end">
                        ...
                        <div class="space-y-3 font-sans">
                            <div class="grid grid-cols-2 items-center gap-2">
                                ...
                                <div class="py-2 px-3 text-xs bg-gray-55 dark:bg-slate-950 border border-gray-200 dark:border-slate-850 rounded-xl text-right text-gray-808 dark:text-white font-bold select-none">à§³{{ number_format($value, 2) }}</div>
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                            ...
                            <div class="grid grid-cols-2 items-center gap-2">
                                ...
                                <input type="number" wire:model.live="discount" class="py-2 px-3 text-xs bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl text-right text-gray-808 dark:text-white font-bold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all">
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦®à§‹à¦Ÿ:</span>
                                <div class="py-2 px-3 text-xs bg-gray-55 dark:bg-slate-950 border border-gray-200 dark:border-slate-850 rounded-xl text-right text-emerald-700 dark:text-emerald-400 font-bold select-none">à§³{{ number_format($grand_total, 2) }}</div>
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦¨à¦—à¦¦:</span>
                                <input type="number" wire:model.live="cash" class="py-2 px-3 text-xs bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl text-right text-emerald-600 dark:text-emerald-400 font-bold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all">
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦¬à¦¾à¦•à¦¿:</span>
                                <div class="py-2 px-3 text-xs bg-gray-55 dark:bg-slate-950 border border-gray-200 dark:border-slate-850 rounded-xl text-right text-red-650 dark:text-red-400 font-bold select-none">à§³{{ number_format($due, 2) }}</div>
                            </div>
                        </div>
                    </div>
