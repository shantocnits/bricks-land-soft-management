<div class="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 px-4 sm:px-6 py-3 rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 transition-colors duration-300">
        <div>
            <h2 class="font-bold text-gray-800 dark:text-white text-sm leading-tight">à¦†à¦œà¦•à§‡à¦° à¦šà¦¾à¦²à¦¾à¦¨</h2>
            <p class="text-[10px] text-gray-400 dark:text-gray-500 font-sans mt-0.5">{{ now()->format('d/m/Y') }} à¦¤à¦¾à¦°à¦¿à¦–à§‡à¦° à¦¸à¦•à¦² à¦šà¦¾à¦²à¦¾à¦¨</p>
        </div>
                ...
                <table class="w-full text-left border-collapse border border-gray-200 dark:border-slate-800" style="min-width: 1050px">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-slate-800 font-sans">
                        @forelse($challans as $challan)
                            <tr class="hover:bg-emerald-50/40 dark:hover:bg-emerald-950/10 transition-colors text-xs">
                                ...
                                <!-- Dropdown Button Teleported to Body to Prevent Table Boundary Clipping -->
                            ...
                            <tr>
                                <td colspan="14" class="px-4 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <p class="text-xs text-gray-400 dark:text-gray-500 italic font-sans">à¦†à¦œà¦•à§‡à¦° à¦•à§‹à¦¨à§‹ à¦šà¦¾à¦²à¦¾à¦¨ à¦ªà¦¾à¦“à¦¯à¦¼à¦¾ à¦¯à¦¾à¦¯à¦¼à¦¨à¦¿à¥¤</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                    ...
                    <tfoot>
                        <tr class="bg-emerald-600/10 dark:bg-emerald-950/30 border-t-2 border-emerald-200 dark:border-emerald-900/50 text-xs font-bold font-sans">
                            ...
                            <td class="px-3 py-3 text-right text-red-600 border-r border-gray-200 dark:border-slate-800 last:border-r-0">à§³{{ number_format($challans->sum('due'), 2) }}</td>
    ...
    <!-- Modal: Add/Edit Challan (styled with exact settings page colors & inputs) -->
    <template x-teleport="body">
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3 mb-6">
                        ...
                        <div class="ml-auto flex items-center gap-2">
                            <div class="flex items-center gap-1.5 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl px-3 py-1.5">
                            ...
                            <input type="date" wire:model="date" class="py-1.5 px-3 text-xs rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-sans font-semibold">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" x-show="$wire.customer_type === 'new'">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="customer_name" placeholder="à¦•à¦¾à¦¸à§à¦Ÿà¦®à¦¾à¦°à§‡à¦° à¦¨à¦¾à¦®"
                                   class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        </div>
                        <div>
                            ...
                            <input type="text" wire:model="customer_address" placeholder="à¦•à¦¾à¦¸à§à¦Ÿà¦®à¦¾à¦°à§‡à¦° à¦ à¦¿à¦•à¦¾à¦¨à¦¾"
                                   class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" x-show="$wire.customer_type === 'old'" x-cloak>
                        <div>
                            ...
                            <select wire:model.live="ledger_id" class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="customer_phone" placeholder="à¦«à§‹à¦¨ à¦¨à¦®à§à¦¬à¦°"
                                   class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                        <div>
                            ...
                            <select wire:model="challan_type" class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        ...
                        <div>
                            ...
                            <input type="date" wire:model="date" class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-sans font-semibold">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5 font-sans">à¦¨à§‹à¦Ÿ</label>
                            <input type="text" wire:model="notes" placeholder="à¦¨à§‹à¦Ÿ..."
                                   class="w-full py-2.5 px-3 rounded-xl border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-xs text-gray-800 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all font-semibold">
                        </div>
                    </div>

                    <!-- Dynamic Table of Items inside Modal (with dynamic Add, Delete, Filter inside row Category Dropdown) -->
                            ...
                            <tbody class="divide-y divide-gray-150 dark:divide-slate-800 font-sans">
                                @foreach($items as $index => $item)
                                    <tr class="text-xs">
                                        ...
                                        <!-- Custom category dropdown with Filter, Add, and Delete inside each row -->
                                        <td class="px-3 py-2.5 border-r border-gray-150 dark:border-slate-850 relative" x-data="{ openCat: false }">
                                            ...
                                            <div x-show="openCat" @click.away="openCat = false" x-transition
                                                ...
                                                <!-- Add New / Filter input inside dropdown -->
                                        ...
                                        <td class="px-3 py-2.5 text-center">
                                            @if(!$loop->first)
                                                <button type="button" wire:click="removeItem({{ $index }})" class="text-red-500 hover:text-red-700 transition-all cursor-pointer">
                                                    ðŸ—‘ï¸
                                                </button>
                                            @endif
                                        </td>
                    ...
                    <!-- Split calculations details grid (Vara field removed, Gari Vara remains) -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 items-end">
                        ...
                        <!-- Right column (Value, Gari Vara, Discount, Grand Total, Cash, Due) -->
                        <div class="space-y-3 font-sans">
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦®à§‚à¦²à§à¦¯:</span>
                                <div class="py-2 px-3 text-xs bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl text-right text-gray-800 dark:text-white font-bold select-none">
                                    à§³{{ number_format($value, 2) }}
                                </div>
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦—à¦¾à§œà¦¿ à¦­à¦¾à§œà¦¾:</span>
                                <input type="number" wire:model.live="transport_rent"
                                       class="py-2 px-3 text-xs bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl text-right text-gray-805 dark:text-white font-bold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all">
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                            ...
                            <div class="grid grid-cols-2 items-center gap-2">
                                ...
                                <div class="py-2 px-3 text-xs bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl text-right text-emerald-700 dark:text-emerald-400 font-bold select-none">
                                    à§³{{ number_format($grand_total, 2) }}
                                </div>
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦¨à¦—à¦¦:</span>
                                <input type="number" wire:model.live="cash"
                                       class="py-2 px-3 text-xs bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl text-right text-emerald-600 dark:text-emerald-400 font-bold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all">
                            </div>
                            <div class="grid grid-cols-2 items-center gap-2">
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">à¦¬à¦¾à¦•à¦¿:</span>
                                <div class="py-2 px-3 text-xs bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl text-right text-red-600 dark:text-red-400 font-bold select-none">
                                    à§³{{ number_format($due, 2) }}
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Modal Actions (e.g. Save, Print, Cancel) -->
                    <div class="flex items-center justify-end gap-2.5 pt-5 border-t border-gray-200 dark:border-slate-800 mt-6">
                        ...
                        <button type="submit" class="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl cursor-pointer transition-all shadow-md active:scale-95 font-sans">
                            à¦¸à§‡à¦­ à¦•à¦°à§à¦¨
                        </button>
                        <button type="button" wire:click="save" class="px-6 py-2 bg-primary hover:bg-emerald-700 text-white text-xs font-bold rounded-xl cursor-pointer transition-all shadow-md active:scale-95 font-sans">
