    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 px-4 sm:px-6 py-3 rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 transition-colors duration-300">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            ...
            <div class="grid grid-cols-2 gap-2 w-full sm:flex sm:items-center sm:gap-3 sm:w-auto">
                ...
                <button type="button" onclick="printChallanArea('pending-challan-print-area')"
                ...
                <table class="w-full text-left border-collapse border border-gray-200 dark:border-slate-800" style="min-width: 1050px">
                                ...
                                <td class="px-3 py-3.5 text-center relative" x-data="{ openDropdown: false, buttonRect: null }">
                                    ...
                                    <template x-teleport="body">
                                        <div x-show="openDropdown" @click.away="openDropdown = false" x-transition
                                            ...
                                            <a href="{{ route('challan.customer-profile', ['phone' => $challan->customer_phone ?: $challan->customer_name, 'from' => 'challan.pending']) }}" class="w-full text-left px-3 py-2 hover:bg-primary-50 dark:hover:bg-primary-950/20 text-gray-700 dark:text-slate-200 hover:text-primary-dark dark:hover:text-primary-400 transition-all font-semibold rounded-xl cursor-pointer flex items-center gap-2">
                    ...
                    <tfoot>
                        <tr class="bg-primary/10 dark:bg-primary-950/30 border-t-2 border-primary-200 dark:border-primary-900/50 text-xs font-bold font-sans">
                            <td colspan="4" class="px-3 py-3 text-primary-dark dark:text-primary-400 border-r border-gray-200 dark:border-slate-800 last:border-r-0">মোট চালান {{ $challans->count() }} টি</td>
                            <td class="px-3 py-3 text-right text-primary-dark dark:text-primary-400 border-r border-gray-200 dark:border-slate-800 last:border-r-0">{{ number_format($challans->sum(fn($c) => $c->items->sum('quantity'))) }}</td>
                            ...
                            @foreach($items as $index => $item)
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-center text-xs bg-white dark:bg-slate-900 md:bg-transparent p-3 md:p-0 rounded-xl border border-gray-100 dark:border-slate-800 md:border-none">
                                    ...
                                    <div class="col-span-4 relative" x-data="{ openCat: false, triggerRect: null }">
                                        <span x-data="{ show: window.innerWidth < 768 }" x-show="show" @resize.window="show = window.innerWidth < 768" x-cloak class="block font-bold text-gray-505 mb-1 text-[11px]">শ্রেণিঃ</span>
                                    ...
                                    <div class="col-span-2">
                                        <span x-data="{ show: window.innerWidth < 768 }" x-show="show" @resize.window="show = window.innerWidth < 768" x-cloak class="block font-bold text-gray-505 mb-1 text-[11px]">রেটঃ</span>
                                    ...
                                    <div class="col-span-2">
                                        <span x-data="{ show: window.innerWidth < 768 }" x-show="show" @resize.window="show = window.innerWidth < 768" x-cloak class="block font-bold text-gray-550 mb-1 text-[11px]">পরিমাণঃ</span>
...
@endif
    ...
    <!-- ====================== PRINT AREA ====================== -->
    <template x-teleport="body">
        <div id="pending-challan-print-area" class="hidden">
            @php
                $printTotal = [
            ...
            <div class="space-y-5 text-gray-800 dark:text-slate-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-2 font-semibold text-xs text-gray-600 dark:text-slate-400">
                    ...
                    <div class="bg-gray-55/50 dark:bg-slate-950/20 border border-gray-150 dark:border-slate-800 rounded-2xl p-4 space-y-2">
                        ...
                        <div class="flex justify-between border-b border-gray-100 dark:border-slate-800 pb-1.5"><span>ধরন</span> <span class="text-gray-800 dark:text-white font-bold">{{ in_array($detailsChallan->challan_type, ['রেগুলার', 'আজকের', 'রেগুলার চালান']) ? 'রেগুলার চালান' : 'অগ্রিম চালান' }}</span></div>
    ...
    <style>
        ...
        @media print {
            ...
            #pending-challan-print-area,
            #pending-challan-print-area * { visibility: visible !important; }
            #pending-challan-print-area {
