...
<aside 
    ...
    class="fixed md:relative inset-y-0 left-0 flex flex-col bg-primary text-white flex-shrink-0 transition-all duration-300 ease-in-out z-40 overflow-visible w-64"
    :class="{ 'translate-x-0': sidebarOpen, '-translate-x-full md:translate-x-0 md:w-20': !sidebarOpen }">
    
    <!-- Logo Area -->
    <div class="flex items-center px-4 h-16 bg-primary-dark border-b border-emerald-800 flex-shrink-0 gap-[10px]">
        ...
        <div class="transition-all duration-300 overflow-hidden whitespace-nowrap flex-grow flex items-center" :class="{ 'opacity-100 max-w-full block': sidebarOpen, 'opacity-0 max-w-0 hidden': !sidebarOpen }">
            <div>
                <h1 class="font-bold text-lg tracking-wider capitalize">bricks <span class="text-emerald-400">land</span></h1>
                <p class="text-[10px] text-emerald-300 font-light truncate">ডেমো ব্রিকস, কলোনী বাজার, লালমনিরহাট</p>
            </div>
        </div>
        ...
        <button 
            ...
            class="md:hidden text-white bg-primary hover:bg-primary-light p-2 rounded-xl focus:outline-none transition-colors duration-150 shadow-sm border border-emerald-800 flex items-center justify-center flex-shrink-0"
    ...
    <div class="flex-grow overflow-y-auto overflow-x-visible py-3 space-y-1 scrollbar-thin scrollbar-thumb-emerald-800 scrollbar-track-transparent">
        ...
        <div class="mx-2">
            <a href="{{ route('dashboard') }}" wire:navigate
               ...
               class="flex items-center px-4 py-2.5 rounded-lg {{ request()->routeIs('dashboard') ? 'bg-secondary text-white shadow-sm' : 'text-emerald-100 hover:bg-emerald-800/50 hover:text-white' }} transition-all duration-200">
        ...
        <div class="mx-2">
            <a href="{{ route('payment-khata') }}" wire:navigate
               ...
               @mouseleave="hideTooltip()"
               class="flex items-center px-4 py-2.5 rounded-lg {{ request()->routeIs('payment-khata') ? 'bg-secondary text-white shadow-sm' : 'text-emerald-100 hover:bg-emerald-800/50 hover:text-white' }} transition-all duration-200">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="mx-2" x-data="{ open: false }">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" class="mt-1 ml-6 space-y-1" :class="{ 'hidden': !sidebarOpen }">
                <a href="#" class="block px-4 py-2 text-xs text-emerald-200 hover:text-white hover:bg-emerald-800/40 rounded transition-all font-sans">বাকি লিস্ট</a>
                <a href="#" class="block px-4 py-2 text-xs text-emerald-200 hover:text-white hover:bg-emerald-800/40 rounded transition-all font-sans">বাকি কালেকশন</a>
            </div>
        </div>
        @endif
        ...
        <div class="mx-2">
            <a href="#" 
               ...
               @mouseleave="hideTooltip()"
               class="flex items-center px-4 py-2.5 rounded-lg text-emerald-100 hover:bg-emerald-800/50 hover:text-white transition-all duration-200">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <div class="p-3 bg-primary-dark border-t border-emerald-800 flex-shrink-0 flex items-center justify-between transition-all duration-300 overflow-hidden">
        <!-- Settings Gear Button -->
        <a href="{{ route('settings') }}" wire:navigate
           @mouseenter="showTooltip('সেটিংস', $el)"
           @mouseleave="hideTooltip()"
           :title="!sidebarOpen ? 'সেটিংস' : ''"
           class="p-2 rounded-lg {{ request()->routeIs('settings') ? 'bg-secondary text-white shadow-sm' : 'text-emerald-100 hover:bg-emerald-850 hover:text-white' }} transition-colors duration-150 flex-shrink-0 flex items-center justify-start flex-grow">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
    ...
    <div 
        ...
        class="absolute left-22 px-3.5 py-2 bg-primary text-white text-xs font-bold rounded-xl shadow-xl whitespace-nowrap z-50 pointer-events-none border border-emerald-700 hidden md:block"
        x-cloak>
    </div>
